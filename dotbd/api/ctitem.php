...
if (isset($_GET['username']) && isset($_GET['password'])){
	...
	$query="select xitemid,xitemcode,xsource,xdesc,xlongdesc,xcat,xbrand,xgitem,xcitem,xunitsale,xtypestk,xmrp,xstdcost,xcp,xdp,xstdprice,zactive,xsup,xstdcost from seitem where zactive=1 and xcitem='All Operation' and xcat like '%$sup%' order by xitemid DESC LIMIT $lim  OFFSET $ofset";
	$result = mysqli_query($db,$query) or die("Cannot execute query");  
	...
	{
		$sup = $_GET['xsup'];
		$query="select xitemid,xitemcode,xsource,xdesc,xlongdesc,xcat,xbrand,xgitem,xcitem,xunitsale,xtypestk,xmrp,xstdcost,xcp,xdp,xstdprice,zactive from seitem where zactive=1 and xcitem='All Operation' and xcat like '%$sup%' order by xitemid";
	$result = mysqli_query($db,$query) or die("Cannot execute query");  
