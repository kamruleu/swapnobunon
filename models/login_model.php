...
class Login_Model extends Model{
	...
	public function run($biz){
		//echo $biz;die;
		$sth = $this->db->prepare("SELECT xusersl, bizid, zemail, zrole, xbranch, xuserrow FROM pausers 
							WHERE zemail = :login and zpassword = :password and bizid = :bizid and zactive=1"); 
		$sth->execute(array(
			':login' => $_POST['login'],
		...
		if($count>0){
			...
			Session::set('susersl', $data['xusersl']);
